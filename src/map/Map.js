...
const resource = "http://brebeufhx5website.pythonanywhere.com/getIndicatorsByCountry";
...
function Map() {
    ...
    map.current.on("load", () => {
      ...
      map.current.on('mousemove', 'country-fills', (e) => {
        if (e.features.length > 0) {
          if (hoveredCountryId !== null) {  // Set previous hover to false
      ...
      map.current.on('click', 'country-fills', (e) => {
        if (e.features.length > 0) {
          if (selectedCountryId !== null) {  // Set previous select to false
            map.current.setFeatureState(
  ...
  async function updateCountryData(countryCode) {
    ...
    .then(response => response.json()
      .catch(_ => {return {"Couldn't fetch data": " "}})
    )
    .then(data => setCountryInfo(data))
    .catch(err => {console.log(err); setCountryInfo({"Couldn't fetch data": " "})})
  }
